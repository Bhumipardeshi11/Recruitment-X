...
class AtsService {
     ...
     * 9. Certifications
     */
    static analyzeResume(resumeText, targetJdText) {
        ...
        let verbCount = 0;
        actionVerbs.forEach(v => { if (textLower.includes(v))
            verbCount++; });
        ...
        const overallAtsScore = Math.round(keywordMatchScore * 0.25 +
        ...
        if (matchedKeywords.length >= 5)
            strengths.push(`High density of key technical terms (${matchedKeywords.slice(0, 4).join(', ')})`);
        if (metricMatches.length >= 2)
            strengths.push('Strong quantitative metric placeholders included in bullet points');
        if (contactScore >= 85)
            strengths.push('Complete contact information with valid email, phone, location, and web links');
        if (formattingScore >= 85)
            strengths.push('Clean ATS-parseable section hierarchy and bullet structure');
    ...
    static async saveAnalysis(resumeId, analysis, jobDescriptionId) {
        const saved = await db_1.prisma.resumeAnalysis.create({
            data: {
                resumeId,
    ...
    static extractKeywordsFromJd(jdText) {
