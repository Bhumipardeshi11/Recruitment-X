...
class GitHubService {
    static async auditProfile(username, userId) {
        try {
            const userRes = await axios_1.default.get(`https://api.github.com/users/${username}`, {
                headers: process.env.GITHUB_TOKEN ? { Authorization: `token ${process.env.GITHUB_TOKEN}` } : {},
                timeout: 5000,
            });
            const reposRes = await axios_1.default.get(`https://api.github.com/users/${username}/repos?per_page=30&sort=updated`, {
                headers: process.env.GITHUB_TOKEN ? { Authorization: `token ${process.env.GITHUB_TOKEN}` } : {},
                timeout: 5000,
            });
            ...
            if (userId) {
                await db_1.prisma.gitHubProfile.upsert({
                    where: { userId },
                    update: {
                        username,
